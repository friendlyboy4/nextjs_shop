...
const LinkContainer = styled(Link)`
  ...
  @media (max-width: 768px) and (min-width: 480px) {
    ${(props) => props.frontpage && "flex-direction: row; font-size: .9rem;"}
  }
`;
...
const GridImageContainer = styled.div`
  ...
  @media (max-width: 768px) and (min-width: 480px) {
    ${(props) => props.frontpage && "flex: 1; margin-right: 1em;"}
  }
  @media (max-width: 480px) {
    ${(props) => props.frontpage && "aspect-ratio: 8 / 5;"}
  }
`;
...
const GridSizeBox = styled.div`
  ...
  z-index: 90;
  @media (max-width: 768px) and (min-width: 480px) {
    ${(props) => props.frontpage && "font-size: 1.6em;"}
  }
  & > p {
    margin: 0;
  }
...
const GridTitle = styled.h3`
  ...
  @media (max-width: 768px) and (min-width: 480px) {
    ${(props) => props.frontpage && "font-size: 2em; font-weight: bold;"}
  }
`;
...
export default function GridItem({ bike, frontpage, custom }) {
  ...
  return (
    <LinkContainer
      ...
      frontpage={frontpage}
    >
      <GridImageContainer frontpage={frontpage}>
        <GridSizeBox frontpage={frontpage}>
          <p>{size}</p>
        </GridSizeBox>
        <Image
      ...
      <GridTextContainer frontpage={frontpage}>
        <GridTitle frontpage={frontpage}>{title}</GridTitle>
        <GridText>{precis}</GridText>
        {!custom && <GridPrice>${price}</GridPrice>}
      </GridTextContainer>
    </LinkContainer>
  );
}
